...
class PicksResponder extends NFLResponder
{
    private $playerId;
    ...
    public function post($args)
    {
        ...
        $data = $this->data;
        $season = $data->getSeasonYear();
        $week = $data->getWeekNumber();
        ...
        HttpHeader::forwardTo($_SERVER['PHP_SELF'] . '?season=' . $season
            . '&week=' . $week);
    }
    ...
    public function get($args)
    {
        ...
        if($this->isLoggedIn())
        {
            $sql = new Query('SELECT p.playerId, p.name, p.bgcolor, 0, 0 FROM player_user AS pu INNER JOIN player AS p ON pu.playerId=p.playerId  WHERE userId=' . $this->user->getId());
            $query = $db->execQuery($sql);
            $player = $db->getRow($query);//*/

            if(array_key_exists($player[0], $players))
                $currPlayer = $players[$player[0]];
            else
                $currPlayer = $player;//s[$this->playerId];
        ...
        ?>
         ...
         <dl class="header-data">
          <dt>Week</dt>
          <dd><?php echo $data->getWeekNumber(); ?></dd>
        ...
        {
            ?>
           <div style="background-color: #eeeeee; margin: 3px; padding: 7px; border: 1px solid #999999; ">
            To verify that your picks are saved, make sure your name appears next to
            each of the teams you chose.
           </div>
            <?php
        }
        else
        {
            ?>
           <div style="background-color: #eeeeee; margin: 3px; padding: 7px; border: 1px solid #999999; ">
            Sign in to select your fantasy picks.
           </div>
            <?php
        }
        ...
        if($weekRow)
        {
            ...
            while($row = $db->getRow($result))
            {
                ...
                if(is_null($lastDate) || $this->getDate($date, 'q') != $this->getDate($lastDate, 'q'))
                {
                    if($lastDate != '')
                    {
                        echo '</table></div>';
                    ...
                    ?>
                     <div class="table-default" style="margin-bottom: 3px; ">
                      <table>
                       <tr>
                        <th colspan="<?php echo 3 + count($players); ?>" style="background-color: #ffffff; "><?php $this->printDate($date, 'l, F j, Y'); ?></th>
                       </tr>
                    <?php
                ...
                if($this->getDate($date, 'g:i a') != $lastTime)
                {
                ...
                else
                {
                    ?>
                     <tr>
                      <td colspan="<?php echo 3 + count($players); ?>" style="background-color: #999999; height: 3px; font-size: 1px; padding: 0; margin: 0; "></td>
                     </tr>
                    <?php
                }
